...
flowchart TD
    ...
    E --> L["SpellCastRuntimeDiagnostics"]
    L --> C
...
- `Game.Character` 本来就依赖 Skills 和 Combat，因此由它负责 Runtime Trace 和 Lab 驱动最合适；其中 `SpellCaster` 只决定何时进入诊断与 Gameplay Spawn，`SpellCastRuntimeDiagnostics` 集中处理开发诊断细节。
...
- 通过 `SpellCastRuntimeDiagnostics` 分配 `castId`，payload 递归仍保留同一 castId 并增加 depth；
- Mana 预检查后调用 Helper 选择普通求值或带 Trace 求值，然后继续消费 `EmitCommand` 并进入 Gameplay Spawn；
- 新增 `ProfilerMarker("Spell.RuntimeSpawn")`；
- 保留 public `TraceLevel` Contract，供 Performance Lab 判断诊断是否会污染性能采样。

#### 新增：`SpellCastRuntimeDiagnostics.cs`

这个普通 C# Helper 持有全部 `UNITY_EDITOR || DEVELOPMENT_BUILD` 条件编译、可复用 `_traceCollector`、Summary/Detailed 日志格式化和 Mana 失败诊断。Release Build 中 public `TraceLevel` 仍表现为 `Off`，而 `SpellCaster.cs` 不再被多段 `#if` 打断。

这里尤其重要的是职责边界：`SpellCaster` 只决定何时调用诊断、何时进入 Gameplay Spawn；`SpellCastRuntimeDiagnostics` 只观察和格式化结果；`CastEvaluator` 的 `Evaluate` 与 `EvaluateWithTrace` 继续共用唯一的 `EvaluateCore`。Mana 不足时的 Detailed Trace 只做纯 `EvaluateWithTrace`，随后清空临时 `output`，绝不能实例化或重复扣费。
...
7. `SpellCastRuntimeDiagnostics.cs`：理解条件编译、Collector 和日志为什么与 Gameplay 主流程隔离。
8. `SpellCaster.cs`：沿着 Mana 预检查、纯求值输出、Runtime Spawn 阅读主流程。
9. `ProjectileDebugSnapshot.cs`：先读数据契约。
10. `ProjectileBase.GetDebugSnapshot` 与生命周期注册：理解事实从哪里来。
11. `ProjectileDebugGizmoDrawer.cs`：理解事实如何转成空间图形。
12. `ProjectileDebugSnapshotTests.cs`：理解生命周期和物理测试。
13. `SpellPerformanceLabController.cs`：最后学习压力状态机，因为它依赖前面所有入口。
...
答：Trace 等级默认 Off，Detailed Collector 和日志逻辑集中在 `SpellCastRuntimeDiagnostics` 中，并受 `UNITY_EDITOR || DEVELOPMENT_BUILD` 条件编译保护；项目日志入口也使用 Conditional。`SpellCaster` 的 Release 路径只进行正常求值与 Spawn。性能基线时关闭 Trace 和 Gizmos，避免 Console 格式化和绘制污染结果。
